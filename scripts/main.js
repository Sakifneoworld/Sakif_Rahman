jQuery(document).ready(function($){
	...
	$timeline_block.each(function(){
		if( $(this).offset().top < $(window).scrollTop()+$(window).height()*0.85 && $(this).find('.cd-timeline-img').is('.is-hidden, .bounce-out') ) {
				$(this).find('.cd-timeline-img, .cd-timeline-content').removeClass('is-hidden bounce-out').addClass('bounce-in');
		}
		if($(this).offset().top >= $(window).scrollTop()+$(window).height()*0.85 && $(this).find('.cd-timeline-img').is('.bounce-in')) {
